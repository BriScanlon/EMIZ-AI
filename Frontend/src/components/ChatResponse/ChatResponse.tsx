...
export default function ChatResponse({
  ...
  // Check if the graph has valid data
  const hasGraphData = graph && Object.keys(graph).length > 0;
    ...
    <div className={classlist}>
      <div className={mc('response__bubble')}>
        ...
        <div className={mc('response__actions')}>
          {/* Graph Button - Disabled if no graph data */}
          <Link
            to={hasGraphData ? `/conversation/${conversationId}/extension/graph/${index}` : '#'}
            className={mc('response__actions-graph')}
            aria-label="Show graph"
            title="Show graph"
            style={{
              opacity: hasGraphData ? 1 : 0.5, // Greyed out if no data
              pointerEvents: hasGraphData ? 'auto' : 'none', // Prevent clicking
            }}
          >
